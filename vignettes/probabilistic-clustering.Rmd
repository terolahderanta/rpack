...
This vignette provides a tutorial on the `pclustr` package. 

## Libraries

First, load the `pclustr` package 
as well as `ggplot2` for plotting the data:
...
we let the sums of all the weights in a cluster vary uniformly within a given range. We set the
...
init_kmpp <- kmpp(cbind(rep(dat$x, dat$w),
                        rep(dat$y, dat$w)),
                  k)
...
plot_clusters(
  ...
  subtitle = paste("Uniform prior on [", L, ", ", U, "]", sep = "")
)
```
...
we let the sums of all the weights in a cluster vary normally, setting the standard deviation as 3.
...
plot_clusters(
  ...
  title = paste("Probabilistic clustering, k = ", k, 
                sep = ""),
  subtitle = paste("Gaussian prior (sd = ", pr_sd, ")", 
                   sep = "")
) 
```
