# This is a simpler version of the probabilistic clustering algorithm, where the constraints fo cluster sizes is given only as a boundary
# from L to U. In the file "prob_clust.R" user inputs the knowledge about cluster sizes as a prior distribution.  
...
prob_clust <- function(data, weights, k, init_mu, L, U, lambda){
  ...
  #   L: lower limit for cluster sizes.
  #   U: upper limit for cluster sizes.  
  # Returns
...
prob_clust_allocation <- function(data_ew, mu, k, L, U, lambda){
  ...
  return(list(apply(matrix(get.variables(lp1)[1:(n * k + n)], ncol = k + 1), 1, which.max), obj_min))
...
prob_clust_allocation_weights <- function(data, weights, mu, k, L, U, lambda){
  ...
  # Parameters
  #   data: matrix or data.frame containing the data, where each object is considered to be equally weighted.
  #   weights: weigths of the objects in data. 
  #   mu: parameters (locations) that define the k distributions.
  #   k: number of clusters.
  #   L: lower limit for cluster sizes.
  #   U: upper limit for cluster sizes.
  ...
  # Objective function in the optimization problem
  set.objfn(lp1, c(C * weights, rep(nu2 * lambda, n)))
  ...
  temp <- c(t(matrix(1, ncol = n, nrow = k) * 1:k))
  for (i in 1:k) {
    add.constraint(lp1, c(as.numeric(temp == i, n) * weights, rep(0, n)), ">=", L)
  }
  
  # Constraint 3: each cluster size must be lower than U 
  for (i in 1:k) {
    add.constraint(lp1, c(as.numeric(temp == i, n) * weights, rep(0, n)), "<=", U)
  }
...
# Simulated data (n = 500)
{
  ...
  # w: weights for objects (must be integers)
  # id: identification for each object
  koord <- data.frame(x = c(rnorm(100, mean = 0.5, sd = 2), rnorm(150, mean = 23.5, sd = 5), 
...
# Simulated data (n = 1000)
{
  ...
  # w: weights for objects (must be integers)
  # id: identification for each object
  koord <- data.frame(x = c(rnorm(250, mean = 0.5, sd = 2), rnorm(250, mean = 21.5, sd = 3), 
...
# Function call prob_clust
{
  lambda1 <- 5
  pt <- proc.time()
  temp <- prob_clust(data = koord[, 1:2], weights = koord$w, k = k, init_mu = init_mu, L = L, U = U, lambda = lambda1)
  koord$cl <- temp[[1]]
  mu <- temp[[2]]
  obj_min <- temp[[3]]
  proc.time() - pt
  plot_clusters(koord[,1:2], koord$w, koord$cl, mu, main = paste("Range: ", L, "-", U, sep = ""))
}
